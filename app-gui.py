...
class MainUI(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for F in (StartPage, PageOne, PageTwo, PageThree, PageFour):
            ...
            frame.grid(row=8, column=8, sticky="nsew")
...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        try:
            while True:
                frame = gif.copy()
                frame = frame.resize((desired_width, desired_height), Image.ANTIALIAS)  # Resize each frame
                rounded_frame = self.round_corners(frame, 40)  # Round corners of the frame
        ...
        frame_index = 0  # Initial frame to display
        self.gif_photos = [ImageTk.PhotoImage(frame) for frame in frames]

        # Create a label widget to show the GIF
        self.img = tk.Label(self)
        self.img.grid(row=0, column=1, rowspan=4, sticky="nsew", pady=200)

        # Update the GIF image periodically to create animation
        def update_gif(index):
            self.img.configure(image=self.gif_photos[index])
            frame_index = (index + 1) % len(frames)  # Move to the next frame
            self.after(100, update_gif, frame_index)  # Update every 100 milliseconds

        update_gif(frame_index)  # Start the GIF animation

        # Rest of your code...
        label = tk.Label(self, text="        Home Page        ", font=self.controller.title_font, fg="#263942")
        label.grid(row=0, sticky="ew", padx=200)
        ...
        button1.grid(row=1, column=0, ipady=3, ipadx=7, padx=200)
        button2.grid(row=2, column=0, ipady=3, ipadx=2, padx=200)
        button3.grid(row=3, column=0, ipady=3, ipadx=32, padx=200)
...
class PageOne(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        self.controller = controller
        tk.Label(self, text="Enter the name", fg="#263942", font='Helvetica 12 bold').grid(row=0, column=0, pady=200, padx=300)
...
class PageTwo(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        global names
        self.controller = controller
        tk.Label(self, text="Select user", fg="#263942", font='Helvetica 12 bold').grid(row=0, column=0, padx=300, pady=200)
...
class PageThree(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.numimglabel.grid(row=0, column=0, columnspan=2, sticky="ew", pady=200, padx=450)
...
class PageFour(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label.grid(row=0,column=0, sticky="ew", padx=200, pady=300)
...
app = MainUI()
app.iconphoto(False, tk.PhotoImage(file='skit.png'))
app.mainloop()
